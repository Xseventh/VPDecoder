...
public sealed record Vp9DequantTables(
    int YDc,
    int YAc,
    int UvDc,
    int UvAc)
{
    public static Vp9DequantTables Create(Vp9QuantizationHeader quantization, int bitDepth)
    {
        if (bitDepth != 8)
        {
            throw new NotSupportedException("Only VP9 8-bit dequantization is supported.");
        }

        return new Vp9DequantTables(
            Vp9Quantization.DcQuant(quantization.BaseQIndex, quantization.YDcDeltaQ),
            Vp9Quantization.AcQuant(quantization.BaseQIndex, 0),
            Vp9Quantization.DcQuant(quantization.BaseQIndex, quantization.UvDcDeltaQ),
            Vp9Quantization.AcQuant(quantization.BaseQIndex, quantization.UvAcDeltaQ));
    }
}

public static class Vp9Quantization
{
    ...
    public static int DcQuant(int qindex, int delta)
    {
        return DcLookup[ClampQIndex(qindex + delta)];
    }

    public static int AcQuant(int qindex, int delta)
    {
